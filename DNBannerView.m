...
//  OBJCDemo
//
//  Created by 帅科 on 2018/8/13.
//  Copyright © 2018年 shuaike. All rights reserved.
//

#import "DNBannerView.h"
#import <ImageIO/ImageIO.h>

#define DEFAULTTIME 4
#define HORMARGIN 10
#define VERMARGIN 5
#define DES_LABEL_H 20
#define DNDEFAULT_PLACEHOLD_IMAGE [UIImage imageNamed:@"icon_nopicture"]
@interface DNBannerView ()<UIScrollViewDelegate>
//轮播的图片数组
...
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DNBannerView"];
    NSLog(@"image cache src :%@",cache);
    BOOL isDir = NO;
...
- (void)setScrollViewContentSize {
    if (_images.count > 1) {
        ...
        if (_changeMode == DNChangeModeFade) {
...
- (void)setPagePosition:(DNPageControlPosition)pagePosition {
    _pagePosition = pagePosition;
    // 如果images只有一张图片或者设置为隐藏时，pageControl需要隐藏
    _pageControl.hidden = (_pagePosition == DNPositionHide) || (_imageArray.count == 1);
    if (_pageControl.hidden) return;
    
    CGSize size;
    if (!_pageImageSize.width) {
        //没有设置图片，系统原有样式
    ...
    if (_pagePosition == DNPositionDefault || _pagePosition == DNPositionBottomCenter)
        // 默认pageControl在底部居中显示
        _pageControl.center = CGPointMake(self.width * 0.5, centerY);
    else if (_pagePosition == DNPositionTopCenter)
        // 顶部居中
        _pageControl.center = CGPointMake(self.width * 0.5, size.height * 0.5 + VERMARGIN);
    else if (_pagePosition == DNPositionBottomLeft)
        // 从底部排列，水平间距10
...
- (void)nextPage {
    ...
    } else [self.scrollView setContentOffset:CGPointMake(self.width * 2, 0) animated:YES];
}
...
- (void)downloadImages:(int)index {
    ...
    if (_autoCache) { //如果开启了缓存功能，先从沙盒中取图片
        ...
        if (data) {
            _images[index] = dnGetImageWithData(data);
            return;
        }
    }
    //下载图片
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        if (!data) return;
        UIImage *image = dnGetImageWithData(data);
        //取到的data有可能不是图片
        if (image) {
...
UIImage *dnGetImageWithData(NSData *data) {
...
+ (id)loadDiskCache{
    NSArray *contents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:cache error:NULL];
    NSMutableArray *localImageArr = [NSMutableArray array];
    for (NSString *fileName in contents) {
        NSString *imageSrc =[cache stringByAppendingPathComponent:fileName];
        UIImage *image = [UIImage imageWithContentsOfFile:imageSrc];
        if (image) {
            [localImageArr addObject:image];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX < self.width ) {
        if (_changeMode == DNChangeModeFade) {
            self.currImageView.alpha = offsetX / self.width;
            self.otherImageView.alpha = 1 - offsetX / self.width;
        } else self.otherImageView.frame = CGRectMake(0, 0, self.width, self.height);
        ...
        if (offsetX <= 0) [self changeToNext];
        
        //向左滚动
    } else if (offsetX > self.width ){
        if (_changeMode == DNChangeModeFade) {
            self.otherImageView.alpha = offsetX / self.width - 1;
            self.currImageView.alpha = 2 - offsetX / self.width;
        } else self.otherImageView.frame = CGRectMake(CGRectGetMaxX(_currImageView.frame), 0, self.width, self.height);
        
        self.nextIndex = (self.currIndex + 1) % _images.count;
        self.otherImageView.image = self.images[self.nextIndex];
        if (offsetX >= self.width * 2) [self changeToNext];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (_changeMode == DNChangeModeFade) return;
    if (scrollView.contentOffset.x >= self.width/2.0 && scrollView.contentOffset.x <= self.width * 1.5) {
        [self.scrollView setContentOffset:CGPointMake(self.width , 0) animated:YES];
    }else{
        [self changeToNext];
    }
}
UIImage *dnGifImageNamed(NSString *imageName) {
    ...
    if (data) return dnGetImageWithData(data);
