...
@class DNBannerView;
typedef void(^DNClickBlock)(NSInteger index);
//pageControl的显示位置
typedef enum {
    DNPositionDefault,           //默认值 == PositionBottomCenter
    DNPositionHide,           //隐藏
    DNPositionTopCenter,      //中上
    DNPositionBottomLeft,     //左下
    DNPositionBottomCenter,   //中下
    DNPositionBottomRight     //右下
} DNPageControlPosition;

//图片切换的方式
typedef enum {
    DNChangeModeDefault,  //轮播滚动
    DNChangeModeFade      //淡入淡出
} DNChangeMode;
/**
 *  代理
 */
@protocol DNCarouselViewDelegate <NSObject>
 ...
 *  @param index        图片索引
 */
- (void)carouselView:(DNBannerView *)carouselView clickImageAtIndex:(NSInteger)index;
 ...
 *  其他属性都有默认值，可不设置
 */
@interface DNBannerView : UIView

#pragma mark 属性
/**
 *  设置图片的切换模式，默认为ChangeModeDefault
 */
@property (nonatomic, assign) DNChangeMode changeMode;
 ...
 */
@property (nonatomic, assign) DNPageControlPosition pagePosition;
...
@property (nonatomic, strong) NSArray *actionUrlArray;
 ...
 */
@property (nonatomic, copy) DNClickBlock imageClickBlock;


/**
 *  代理，用来处理图片的点击
 */
@property (nonatomic, weak) id<DNCarouselViewDelegate> delegate;
